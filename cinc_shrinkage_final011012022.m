...
SNtotal_F1 = zeros(75,6);
SNtotal_MAE = zeros(75,6);
SNtotal_PPV = zeros(75,6);
SNtotal_SE = zeros(75,6);
SNtotal_time = zeros(75,6);
    ...
    for ch_i =1%:3
        for ch_i2 = 4%(ch_i+1):4
            ...
            for i = 1:length(alpha_ind)
                ...
                Om = real(Om); Om_real = real(Om_real);
            ...
            [Om_SN,Om_real_SN] = ECG_shrinkage0_color( x0,x0_real, mbeats, 'SN');   
            I2_orig_real = x0_real-Om_real_SN;
            I2_orig = x0-Om_SN;
            %find fECG peak locations with fusion mECG
            [~,~, fbeats_SN, ~, ~, ~, tfrrF, HR_fe, tfrtic, t] = fbeats_extract_real(I2_orig,I2_orig_real,fs,basicTF, advTF, cepR, P, lam_curve, lam_beat, 0, HR_ma_fus);
            ...
            fbeats_SN = fbeats_SN(fbeats_SN>2000); fbeats_SN = fbeats_SN(fbeats_SN<58000);
            ...
            if F1_m>F1_f
                ...
                [fbeats_SN,~] = deal(mbeats, fbeats_SN);
            ...
            [F1_SN,MAE_SN,PPV_SN,SE_SN,TP_SN,FN_SN,FP_SN] = Bxb_compare(gtbeats, fbeats_SN, 50);
            SNtotal_F1(sub_i, PAIRidx) = F1_SN ;
            SNtotal_MAE(sub_i, PAIRidx) = MAE_SN;
            SNtotal_PPV(sub_i, PAIRidx) = PPV_SN;
            SNtotal_SE(sub_i, PAIRidx) = SE_SN;
            SNtotal_TP(sub_i, PAIRidx) = TP_SN;
            SNtotal_FN(sub_i, PAIRidx) = FN_SN;
            SNtotal_FP(sub_i, PAIRidx) = FP_SN;
            total_time(sub_i, PAIRidx) = toc;
...
%save(['cinc_shrinkage_ch2_F1_103022_',OptimalShrinkageOpt],'total_F1', 'total_MAE', 'total_PPV', 'total_SE','total_TP','total_FN','total_FP','total_time','alpha_c');
%save('cinc_shrinkage_ch2_signals_040922','final_mbeats','final_fbeats','final_aECG','final_Om','final_Of','final_I2_orig');
%save(['cinc_shrinkage_ch2_F1_103022_SN'],'SNtotal_F1', 'SNtotal_MAE', 'SNtotal_PPV', 'SNtotal_SE','SNtotal_TP','SNtotal_FN','SNtotal_FP','SNtotal_time','alpha_c');
